<template id="n-form-checkbox">
	<div class="n-form-component n-form-checkbox" :class="{ 'n-form-hidden': hide, 'n-form-disabled': disabled }" :optional="hide != null">
		<slot name="top"></slot>
		<slot name="label" :toggle="function() { toggleValue();$event.stopPropagation() }"><span class="n-form-label" @click="toggleValue();$event.stopPropagation()" v-if="label">{{ label }}</span></slot>
		<div class="n-form-checkbox-content">
			...
			<slot name="overlay" :toggle="function() { toggleValue();$event.stopPropagation() }"><label @click="toggleValue($event);$event.stopPropagation()" class="n-form-checkbox-label"></label></slot>
		</div>
	</div>
</template>