	...
	<div class="is-form-qr n-form-component" :class="{'is-qr-scanned': scanned, 'is-scanned': scanned}">
		...
		<div class="n-form-qr-container" v-show="!switchQrCode || !code">
			<div class="n-form-canvas-container">
				...
				<div ref="overlay" class="overlay" v-show="!scanning" ><button type="button" class="is-button n-form-qr-retry" :class="buttonClass" @click="rescan"><span class="icon" :class="icon" v-if="icon"></span><span class="label">{{ buttonLabel ? buttonLabel : "%{Scan Code}" }}</span></button></div>
			</div>
		</div>
		<div v-if="switchQrCode && code" class="n-form-qr-code" :style="{'width':canvasWidth + 'px', 'height': canvasHeight + 'px'}">
			...
			<div v-show="!scanning" class="actions"><button type="button" class="is-button n-form-qr-retry" :class="buttonClass" @click="rescan"><span class="icon" :class="icon" v-if="icon"></span><span class="label">{{ buttonLabel ? buttonLabel : "%{Scan Code}" }}</span></button></div>
		</div>
