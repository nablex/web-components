<template id="n-input-file">
	<div class="n-input-file" @paste="pasteFiles($event)" @drop="selectFiles($event)" @mouseout="dragging = false" :class="{ 'drag-drop': hasDropSupport(), 'drag-over': dragging }" @dragenter="dragging = true" @dragover="$event.preventDefault()" @dragleave="dragging = false" :disabled="disabled">
		<slot :browse="browse">
			<div v-if="browseLabel || dropLabel" class="upload-zone">
				...
				<div class="selected-files" v-if="value.length"><div class="selected-file" v-for="file in value">{{file.name ? file.name : '%{file:unnamed}'}}<span :class="deleteIcon" :disabled="disabled" class="icon" @click="removeFile(file)"></span></div></div>
			</div>
